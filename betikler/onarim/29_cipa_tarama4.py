# -*- coding: utf-8 -*-
"""29_cipa_tarama4.py — ONARIM 19: taramanın olgu eşleşmesi LEMMA düzeyine taşındı.

v3 ARIZASI (aday 923): olgu alanı KÖK düzeyinde eşleniyordu. Kök olgu alanında
görünüyor ama ayetteki lemma başka anlam alanında olunca yanlış pozitif çıkıyor.
Sûre 27-29'daki on yanlış pozitifin dördü bu sebeple:
  28:10 صبح (lemma أَصْبَحَ = yardımcı fiil, 'sabah' değil)
  28:25 حيي (lemma اسْتِحْياء = utanma) + ظلم (lemma ظالِم = zalim)
  28:50 ظلم (zulüm)
  28:82 صبح (yardımcı fiil) + قدر (lemma قَدِير = kudret)
  29:24 نور (lemma نار = ateş, anlatı unsuru)
  29:53 سمو (lemma مُسَمًّى = adlandırılmış, 'gök' değil)

ONARIM: OLGU kümesi (kök, lemma) çiftlerine taşındı — `28_olgu_lemma.py`.
v3 KORUNUR; bu ayrı bir betiktir ve çıktısı cipa_tarama4_adaylari.json.
...
_ol = import_module('28_olgu_lemma') if False else None
import json as _json, unicodedata as _u
_OL = _json.load(open('olgu_lemma.json', encoding='utf-8'))
OLGU = set(_OL['cok_lemmali']) | set(_OL['tek_lemmali'])
OLGU_LEMMA = {k: set(v) for k, v in _OL['cok_lemmali'].items()}
TEK_LEMMALI = set(_OL['tek_lemmali'])
...
# ayet -> o ayette OLGU ANLAMINDA geçen kökler (lemma süzgecinden geçmiş)
import re as _re
AYLEM = collections.defaultdict(set)
for ln in open('morph.txt', encoding='utf-8'):
    p = ln.rstrip('\n').split('\t')
    if len(p) < 4:
        continue
    ...
    if len(loc) != 4:
        continue
    m = _re.search(r'ROOT:([^|]+)', p[3])
    if not m:
        continue
    l = _re.search(r'LEM:([^|]+)', p[3])
    AYLEM[(int(loc[0]), int(loc[1]))].add((m.group(1), l.group(1) if l else '?'))

# AYKOK: ayetin BÜTÜN kökleri — işaret ailelerinin (رأي, حسب ...) okuduğu küme.
# v3 ile birebir aynı; lemma süzgeci buraya UYGULANMAZ.
AYKOK = collections.defaultdict(set)
for x in AKIS:
    if x['kok']:
        AYKOK[tuple(x['key'])].add(x['kok'])

# AYOLGU: yalnız OLGU ANLAMINDA geçen kökler — lemma süzgecinden geçmiş.
# v4'ün onarımı burada: olgu eşleşmesi kök yerine (kök, lemma) düzeyinde.
AYOLGU = collections.defaultdict(set)
for k, cift in AYLEM.items():
    for kok, lem in cift:
        if kok in TEK_LEMMALI:
            AYOLGU[k].add(kok)                      # tek lemmalı: ayrım gereksiz
        elif kok in OLGU_LEMMA and lem in OLGU_LEMMA[kok]:
            AYOLGU[k].add(kok)                      # çok lemmalı: LEMMA süzgeci
...
json.dump([{'ayet': '%d:%d' % k, 'kokler': ort, 'isaretler': im, 'cipa': k in set(CIPA)}
          ...
          open('cipa_tarama4_adaylari.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
print('\ncipa_tarama4_adaylari.json yazıldı (%d kayıt)' % len(aday))
